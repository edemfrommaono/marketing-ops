...
async function saveSettingsAction(formData: FormData) {
  ...
  await updatePlatformSettings({
    platformName:  formData.get("platform-name")  as string,
    platformUrl:   formData.get("platform-url")   as string,
    supportEmail:  formData.get("support-email")  as string,
    requireInternal: formData.get("require-internal") === "on",
    requireClient:   formData.get("require-client")   === "on",
    autoArchive:     formData.get("auto-archive")     === "on",
    dualApproval:    formData.get("dual-approval")    === "on",
  });
      ...
      <form action={saveSettingsAction} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-soft p-6">
          ...
          <div className="space-y-4">
            {[
              { id:"platform-name", label:"Nom de la plateforme", defaultValue:"Maono Ops — Editorial", type:"text"  },
              { id:"platform-url",  label:"URL de la plateforme", defaultValue:"https://ops.maono.com",  type:"url"   },
              { id:"support-email", label:"Email de support",     defaultValue:"ops@maono.com",           type:"email" },
              ...
              <div key={id} className="flex items-start justify-between gap-4 py-2 border-b border-slate-50 last:border-0">
                ...
                <label className="flex-shrink-0 cursor-pointer">
                  <div className="relative">
                    <input type="checkbox" name={id} defaultChecked={checked} className="sr-only peer" />
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-soft p-6">
          ...
          <div className="space-y-3">
            {[
              { label:"Durée de session (JWT)", name:"session-duration", value:"30 jours", options:["1 jour","7 jours","30 jours","90 jours"] },
              { label:"Délai d'expiration des liens client (portail)", name:"portal-expiry", value:"7 jours", options:["24h","48h","7 jours","30 jours"] },
            ].map(({ label, name, value, options }) => (
              <div key={label} className="flex items-center justify-between gap-4">
                <label className="text-sm text-anthracite">{label}</label>
                <div className="relative">
                  <select name={name} defaultValue={value} className="appearance-none px-4 py-2 pr-8 border border-slate-200 rounded-lg text-sm outline-none focus:border-editorial bg-white">
        ...
        <div className="bg-red-50 rounded-xl border border-red-200 p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button type="button" className="px-3 py-1.5 border border-red-300 text-red-700 rounded-lg text-xs font-semibold hover:bg-red-100 transition-colors">
                Purger
              </button>
            </div>
            <div className="flex items-center justify-between">
        ...
        <div className="flex justify-end gap-3 pt-2">
          <a href="/admin/settings" className="px-5 py-2.5 border border-slate-200 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-50">
            Annuler
          </a>
          <button type="submit" className="px-5 py-2.5 bg-editorial text-white rounded-lg text-sm font-semibold hover:bg-editorial/90 shadow-sm">
